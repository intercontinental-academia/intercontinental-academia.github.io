<template>
  <v-app dark>
    <v-overlay :value="error" class="text-center headline">
      ...
      <br />
      <v-icon v-if="error.statusCode === 404" x-large class="mb-3">
        mdi-link-variant-off
      </v-icon>
      <v-icon v-else x-large class="mb-3"> mdi-ladybug </v-icon>
      <br />
      <template v-if="error.statusCode === 404">
        This page does not exist. <br />
        <div class="body-1 mt-3">
          <NuxtLink to="/">
            Go back to the Intercontinental Academia Home page
          </NuxtLink>
        </div>
      </template>
      <template v-else>
        An error happened while loading this page. <br />You can try again and
        refresh this page or <a href="mailto:">contact us</a
        ><!-- TODO COmplete -->
